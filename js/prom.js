// js/prom.js - Модуль рапорта о повышении (05-REP-PROM)
class PromReportModule {
    ...
    setupListeners() {
        ...
        document.getElementById('promo-basis')?.addEventListener('change', (e) => {
            this.toggleBasisBlocks(e.target.value);
        });
        ...
        document.getElementById('promo-service2')?.addEventListener('change', (e) => {
            this.updateSpecializedGrades(e.target.value);
        });
    }
    ...
    toggleBasisBlocks(value) {
        ...
        if (value === 'operation') {
            operationBlock.classList.remove('hidden');
            eventBlock.classList.add('hidden');
        } else if (value === 'event') {
            operationBlock.classList.add('hidden');
    ...
    togglePromotionType(value) {
        ...
        if (value === 'combat') {
            combatBlock.classList.remove('hidden');
            specializedBlock.classList.add('hidden');
        } else if (value === 'specialized') {
            combatBlock.classList.add('hidden');
            specializedBlock.classList.remove('hidden');
        } else {
            combatBlock.classList.add('hidden');
            specializedBlock.classList.add('hidden');
        }
    ...
    updatePreview() {
        ...
        const service1 = document.getElementById('promo-service1')?.value || '[НЕ ЗАПОЛНЕНО]';
        ...
        const service2 = document.getElementById('promo-service2')?.value || '[НЕ ЗАПОЛНЕНО]';
        ...
        preview += `**[ТЕКУЩИЙ СПЕЦИАЛИЗИРОВАННЫЙ ГРЕЙД]**: ${service1}\n`;
        ...
        preview += `**[ТЕКУЩИЙ СПЕЦИАЛИЗИРОВАННЫЙ ГРЕЙД]**: ${service2}\n\n`;
        preview += `———————————————————————\n\n`;
        ...
        if (promoType === 'combat') {
            ...
            preview += `[ ] О повышении специализированного грейда\n`;
        } else if (promoType === 'specialized') {
            preview += `[ ] О повышении основного боевого грейда\n`;
            preview += `[X] О повышении специализированного грейда: ${service2} → ${specializedGrade || '[НЕ ВЫБРАН]'}\n`;
        } else {
            preview += `[ ] О повышении основного боевого грейда\n`;
            preview += `[ ] О повышении специализированного грейда\n`;
        }

        preview += `[ ] О присвоении первичного специализированного грейда: [ОТСУТСТВУЕТ] → [НОВЫЙ]\n\n`;
        ...
        document.getElementById('report-preview').innerHTML = preview.replace(/\n/g, '<br>');
    }
}